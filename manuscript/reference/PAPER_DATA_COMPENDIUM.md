...
# EvalShift Paper Data Compendium
...
| **EvalShift / distributional retrieval** | energy, MMD, sliced-Wasserstein, coverage | energy/MMD regret (A); MoA-nDCG, coverage (B) | A + B | this work; Far-from-Straightforward 2026; Wei 2026 |
...
every family. The circularity risk is field-wide, not specific to EvalShift or to
expression-based methods.

### 2.2 Baselines run head-to-head against EvalShift
...
| `cmap_cosine` | CMap-style cosine | exp08 | zero-variance limit of EvalShift (identity proof) |
...
EvalShift's `global_energy` leads; the two CMap/mean scorers are numerically identical.

| method | Hit@1 | Hit@5 | MRR | nDCG@10 | class |
|---|---|---|---|---|---|
| **global_energy** (EvalShift) | **0.8369** | 0.9726 | 0.9018 | 0.9215 | A |
| pca_dist | 0.7782 | 0.9603 | 0.8610 | 0.8919 | A |
| coverage_mean (EvalShift) | 0.7734 | 0.9504 | 0.8565 | 0.8841 | A |
| coverage_worst (EvalShift) | 0.5893 | 0.8770 | 0.7253 | 0.7696 | A |
...
Same rankers, but candidates are generated by predictors. EvalShift's distributional
...
nDCG deltas (EvalShift minus mean, per predictor): average_effect **-0.004**,
...
Among unsupervised signals, EvalShift's distance-reduction is the best (behind only the
...
| **distance_reduction** | **EvalShift distributional** | **1.0** | 0.11 | **1.0** | **0.431** |
...
EvalShift is the best unsupervised ranker on both the drug side (Hit@5 = 1.0) and the
...
| sanity: predicted_mean EvalShift-mean gap | 0.0507 (pass) |
...
|---|---|---|
| 1 Method unification | "CMap cosine retrieval is the zero-variance limit of distribution-aware retrieval" | "all CMap methods identical to EvalShift"; "WTCS mathematically identical" |
| 2 Information-conditioned advantage | "EvalShift's advantage depends on the information condition of the candidate source" | "EvalShift improves any perturbation predictor" |
| 3 HIR-Bench | "failure risk is moderately predictable from observable features" | "failure risk is accurately predicted" |
| 4 Boundary audit | "under an energy proxy EvalShift reduces regret; under oracle-independent metrics the advantage is not yet demonstrated" | "EvalShift provides better therapeutic recommendations" |
| 5 Exploratory biology | "response divergence may mark resistance-associated minority states" | "EvalShift predicts resistance"; "identifies persister cells" |

Global rule: EvalShift is a reliability-aware evaluation probe, not a therapeutic
...
**Evaluation-critique tier (bounds EvalShift's own scores)**
