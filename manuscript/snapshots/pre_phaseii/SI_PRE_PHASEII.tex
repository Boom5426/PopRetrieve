...
\newcommand{\MAINRECOVER}{Fig.~5e}   % recoverability ceiling vs best unsupervised
...
supporting experiments for the main text. It contains four Supplementary Notes and four
Supplementary Tables. It carries no Extended Data figures: all 21 panels of the former Extended
Data deck are now in the main figures. The supplementary analyses follow the same
...
The main text reports recoverability in the constructed two-state mixture as a single supervised
ceiling against the best unsupervised method (\MAINRECOVER). Three further measurements
characterise the same limit and are reported here rather than in the figure.
...
0.106, was obtained when silhouette selection chose $k=4$. No configuration approached the value
of 0.5 conventionally treated as reliable recovery, so the low ceiling is not specific to the
clustering method used in the main analysis.
...
\paragraph{Power of the two task-proximal comparisons in the highest-divergence stratum.}
A null result invites the objection that the comparison could not have detected an effect, so we
computed achieved power and the sample size required for 80\% power directly, in the highest
response-divergence quartile: the stratum in which the pre-specified diagnostic itself predicts the
largest population-level advantage, and therefore the only stratum in which the objection has
force. Two task-proximal criteria were compared on the same queries. For minority-state coverage
the observed mean gap was $+0.0056$ with a standard deviation of $0.0135$ across $n=191$ queries,
giving an achieved power of $1.00$ and requiring only 45 queries for 80\% power. For
mechanism-of-action nDCG the observed mean gap was $+0.0032$ with a standard deviation of $0.163$
across $n=143$ queries, giving an achieved power of $0.06$ and requiring 20,844 queries for 80\%
power.

The two gaps differ by less than a factor of two; the two standard deviations differ by a factor of
twelve. The mechanism-recovery comparison is therefore uninformative at the sample size available,
and this is a property of that metric's variance rather than of the effect size. The correct reading
is asymmetric and we state it as such: the minority-state comparison is well powered and returns a
small effect, which is evidence of a small effect; the mechanism-recovery comparison is not
adequately powered, so its null is an absence of evidence rather than evidence of absence. The main
text's mechanism-recovery conclusion rests on the full 600-query set and on the shape of the whole
distribution (\MAINCLASSAB), not on this stratum.

These figures were reported in the main text until 2026-08-31, when the two panels carrying them
were moved here to keep the main figure to the evidence that carries its argument.
...
The pre-specified diagnostic declines 133 queries as ``mean or no call'' and a further 11 as
``mean sufficient''. The 11 mean-sufficient queries are excluded from the main comparison because
their regret reduction is exactly 0.000 by construction; pooling them into the non-recommended
group changes nothing, giving a difference of $+0.005$ with a Mann--Whitney $p=0.81$ and an
interval covering zero.

Pooling the two decline verdicts into one group returns $p=0.09$, which reads as an absence of
effect but is two opposite effects cancelling: the 133 ``mean or no call'' queries are more
divergent than the queries the diagnostic recommends, while the 11 ``mean sufficient'' queries lie
below the fifth percentile of both groups, where the diagnostic is correct. A distributional test
on the same pooling detects the difference the location test misses
...
\paragraph{Two reference baselines, and why both are reported.}
...
case $g=\mathrm{id}$ and the latent case in which $g$ is the model's own reconstruction, and it
is asserted rather than fitted. Referring it to the real
...
so its two references coincide at 1.000, which is the correct answer for such a model rather
than a defect of the measurement. The other three pass their output through a decoder whose
...
The gap also survives two checks that remove the two obvious ways it could be an artefact of
aggregation. Fixing every probe--clusterer pair, so that the same pair is scored in both arms,
