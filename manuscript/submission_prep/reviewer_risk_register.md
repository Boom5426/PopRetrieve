...
| 3 | "Isn't the Class-A regret gain enough to claim usefulness?" | high | Result 2 label; Result 3; negative-claims box | It is explicitly a Class A objective-aligned proxy, and after the 2026-09-03 estimator reissue it is **+0.031** (56% of queries improved, p = 1.6e-12), not the +0.119 earlier drafts quote. Against a magnitude-aware reference the distributional part of it is exactly zero. **The paired contrast once quoted here is RETRACTED:** the positive and negative numbers came from different scorers on different query sets (R4), and "0 of 37" was the QUICK sanity task count quoted as a real-data result (R13). Use the Results section's current Class A / Class B statement. |
