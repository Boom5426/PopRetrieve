...
# PopRetrieve Manuscript — Statistical Reporting Checklist
...
Metric-class legend:
- **circular-risk** — outcome aligned with PopRetrieve's own objective (energy/distributional).
  Gains are suggestive, not conclusive.
- **proxy** — synthetic/oracle-based welfare; internally valid, externally unvalidated.
- **independent** — outcome PopRetrieve does not directly optimize (MoA recovery, minority
...
| effect size | PopRetrieve−mean nDCG delta: −0.004 / −0.097 / −0.029 |
| test | paired comparison within predictor (delta of matched queries) |
| metric class | **independent-ish** — nDCG on real-drug candidates; PopRetrieve does not optimize it |
| caveat | Hit@1 shows PopRetrieve winning on 2/3 predictors — report the nDCG delta *and* the mixed Hit@1 (Supp. S4); do not summarize as "all negative" |
...
| metric class | **independent** (structural properties, no PopRetrieve objective involved) |
...
| independent criterion | **0 / 37 tasks PopRetrieve-dominant** under minority-coverage outcome |
| metric class | **independent** (minority coverage, not PopRetrieve's objective) |
...
| metric class | **exploratory / independent** — marker enrichment; no PopRetrieve objective |
