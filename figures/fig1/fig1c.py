"""JUDGE Figure 1 panel 1c: two candidates that share a mean and differ in distribution.
...
def draw_1c(ax):
    ...
    ax.scatter(cand_b[:, 0], cand_b[:, 1], s=6, color=COMP, alpha=0.60,
               edgecolors="none", zorder=3)
    ax.scatter(cand_a[:, 0], cand_a[:, 1], s=6, color=FOCAL, alpha=0.60,
    ...
    curves = [(tgt[:, 0], GREY, "target", 0.9),
              (cand_a[:, 0], FOCAL, "candidate A\ndistribution-matched", 1.1),
              (cand_b[:, 0], COMP, "candidate B\nmean-matched", 1.1)]
    ...
    ax.text(-1.44, y_rule + BAND_H * 0.90, "candidate A\ndistribution-matched", ha="center",
            va="bottom", fontsize=5.8, color=FOCAL, linespacing=1.15)
    ax.text(m[0] + 0.16, y_rule + BAND_H + 0.10, "candidate B\nmean-matched", ha="left",
            va="bottom", fontsize=5.8, color=COMP, linespacing=1.15)
