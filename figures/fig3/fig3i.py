"""PopRetrieve Figure 3 panel 3i: Class C per query, distributional score against the scalar it must beat.
...
(drug-drug functional similarity from GDSC2 dose-response profiles; see fig3h.py).
...
Run standalone: python fig3i.py
"""
import os
...
def draw_3i(ax):
    ...
    ax.fill_between([lo, hi, xhi], [lo, lo, lo], [lo, hi, hi],
                    color=FOCAL_SOFT, alpha=0.06, zorder=0, lw=0)

    for ln, mk in zip(LINES, MARKS):
        s = d[d.cell_line == ln]
        ax.plot(s.energy_rho, s.magnitude_match_rho, ls="", marker=mk, ms=3.2,
                mfc=FOCAL_SOFT, mec="white", mew=0.35, alpha=0.80, label=ln, zorder=3)
    ...
    # the Fig. 3 caption. The statistic is still computed here and printed on a standalone run so
...
if __name__ == "__main__":
    ...
    fig.savefig(os.path.join(os.path.dirname(__file__), "3i.png"), dpi=200, bbox_inches="tight")
    print("wrote 3i.png")
