"""Figure 6 panel 6a: predict-then-rank schematic, with the three conditions named.
...
RE-CUT 2026-07-26 for the 1:1 canvas. Figure 6 is now authored at its printed width (6.9 in) and
...
rests on evidence collected to answer other questions (Fig. 5f, Fig. 4h,i), chief among it a
...
def draw_6a(ax):
    ...
    def box(x, y, w, h, txt, ec, fc, fs=6.0, tc=None, lw=0.9, ls="solid"):
        ax.add_patch(mpl.patches.FancyBboxPatch(
            (x - w / 2, y - h / 2), w, h, boxstyle="round,pad=0.010",
            fc=fc, ec=ec, lw=lw, ls=ls, zorder=2))
        ax.text(x, y, txt, ha="center", va="center", fontsize=fs,
                color=tc or ec, zorder=3, linespacing=1.25)

    def arrow(x0, y0, x1, y1, c=GREY, lw=0.9):
...
if __name__ == "__main__":
    fig, ax = plt.subplots(figsize=(1.33, 1.42))   # the slot it occupies in fig6_assemble
    draw_6a(ax)
