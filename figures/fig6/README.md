# Figure 6: The information condition is a two-gate criterion
...
**Authority.** The Figure 6 caption in `manuscript/latex/EvalShift_manuscript.tex` is the authority for
...
are documented in `fig6_assemble.py`; that docstring, not this one, is the place layout decisions
are recorded.

## Panels (mechanism chain: predict -> preserve -> identify -> decide)

Titles below are exactly the strings in `fig6_assemble.TITLES`, which is what the composite
...
| b | Gain is small and sign-inconsistent | nDCG@10 gain (EvalShift energy minus mean cosine) per predictor family: $-0.071$ nearest-neighbour, $-0.016$ average-effect, $+0.027$ latent-linear. One task, so no dispersion is available; bars are coloured by sign. | `results/exp09_predict_then_rank/summary.csv` |
...
- **e is scoped to the constructed mixture on purpose, and its general reading is withdrawn.**
  ...
  tissue Gate 2 is an algorithmic bottleneck, not an informational one** (main-text Fig. 5d;
...
- `fig6a.py` ... `fig6g.py` : per-panel draw functions, each runnable standalone
  (`python fig6c.py`). The `__main__` block of each writes a scratch `6*.png` preview; the
  composite is not built from those PNGs.
- `fig6_assemble.py` : the 7-panel, 3-row layout (a|b|c / d|e / f|g), authored at the FINAL PRINTED
...
- `fig6_two_gate.{pdf,svg,png}` : the composite, and the only stem this figure is written under.
  `python figures/build_all.py --write` enforces the 5 pt floor, writes these, and copies the PDF
  to `manuscript/latex/figures/fig6.pdf`, which is the file the manuscript compiles.
