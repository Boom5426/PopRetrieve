...
def draw_nat_gate1(ax):
    ...
    _retune(ax, {
        ...
        # One line, not two. Right-aligned at the spine, the two-line form reached back across
        # the point column AND its outer edge touched panel f's rotated y label in the gutter
        # between the two panels. "cos = 1" restates the y axis, which is already a cosine and
        # already ticked at 1.0, so the half that carries meaning is the half that stays.
...
def draw_nat_premise(ax):
    ...
    hits[0].set_text(hits[0].get_text()
                     .replace(" of them from one patient", " from one patient"))
