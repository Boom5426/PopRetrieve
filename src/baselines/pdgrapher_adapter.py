"""PDGrapher adapter — bridge inverse-design target ranking <-> PopRetrieve drug ranking.
...
the state toward a desired one, using network/graph proximity. PopRetrieve instead ranks candidate
...
def drug_ranking_to_target_ranking(drug_scores: dict[str, float], dt_map: DrugTargetMap,
                                   ...
                                   agg: str = "max") -> dict[str, float]:
    """Score each target by aggregating the scores of the drugs that hit it (PopRetrieve -> targets)."""
    aggf = np.max if agg == "max" else np.mean
    # build target set
    if targets is None:
