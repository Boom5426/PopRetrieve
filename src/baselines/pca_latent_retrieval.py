...
A representation-learning stand-in between raw-signature CMap retrieval and EvalShift's full
...
retrieve with one of two kernels:
    ...
    mode='dist'  — a distributional distance IN the latent space: the (negated) energy
                   ...
                   a cheap distributional retriever to bracket EvalShift against.
...
def _energy_distance_np(X: np.ndarray, Y: np.ndarray, max_cells: int = 400,
    ...
    (the ambient-space EvalShift energy uses the torch/CUDA kernel — this is the latent bracket).
    """
