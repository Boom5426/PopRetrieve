...
Establishes whether EvalShift's required information condition — candidate response
populations partially observed, optimal drug hidden — holds in real single-cell
drug screens, and whether EvalShift reduces decision regret / improves nDCG in the
...
# 4-key lookup returns three rows and taking .iloc[0] pairs a EvalShift row at fraction 0.4
...
def _recommendation_vs_outcome(df_perf):
    """Stratify EvalShift-mean advantage by recommendation_mode (protocol §5 core test)."""
    rows = []
        ...
        for dart_m in DART_METHODS:
            ...
            for _, r in dm.iterrows():
                ...
                if key not in ref.index:
                    ...
                    continue
                rr = ref.loc[key]
                deltas_regret.append(rr.decision_regret - r.decision_regret)  # + = EvalShift better
...
def run(quick=False, n_drugs=None, n_seeds=None):
    ...
    section("EvalShift vs MEAN BY RECOMMENDATION MODE")
    if len(df_rvo) > 0:
        for rmode in df_rvo.recommendation_mode.unique():
            ...
            log(f"  [{rmode}] best EvalShift={best.dart_method}: "
