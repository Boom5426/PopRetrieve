...
Three parts:
  ...
  C  method-contrast: does the JUDGE-top perturbation cover the resistance-associated
...
# ── Part C: JUDGE-top vs mean-top minority rescue ─────────────────────────────

def run_part_C(ds, contexts, min_cells=40, seed=0, n_query=6):
    """Does the JUDGE-selected KO cover the divergent resistance minority state that the
    ...
    for ctx in contexts:
        ...
        for ko in top_kos:
            ...
            # mean-cosine top vs JUDGE-energy top
...
def run(quick=False):
    ...
    # Part C: JUDGE vs mean minority rescue
    ...
    section("PART C — JUDGE-top vs mean-top minority rescue")
    if len(dfC) > 0:
        log(f"  queries: {len(dfC)}, different pick: {int((~dfC.same_pick).sum())}/{len(dfC)}")
        log(f"  mean JUDGE minority rescue: {dfC.dart_minority_rescue.mean():+.4f} "
            f"(>0 => JUDGE covers resistance minority better)")
        log(f"  frac queries JUDGE rescues: {(dfC.dart_minority_rescue > 0).mean():.2f}")
