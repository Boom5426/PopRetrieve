...
Acceptance table (protocol §; the projection is "accepted" if these hold):
  CD34+ / low-conflict            -> HIR predicts mean_sufficient          (DART no win)
  exp09 mean-only predictors      -> HIR predicts no-DART / mean_sufficient
...
The HIR-Bench boundary (derived from results/exp11_hir_benchmark):
  - low/mid preference_conflict  -> mean and DART tie (regret ~0)  -> mean_sufficient
  - high preference_conflict + reliable structure -> DART lowers regret -> DART_recommended
  - predicted_mean information condition -> DART advantage shrinks -> no-DART
...
def fit_hir_boundary():
    """Derive the conflict threshold at which DART starts beating mean (worst
    ...
    # For each grid cell, min regret per family; DART advantage = mean_regret - dart_regret
    ...
    # DART wins above a conflict threshold. Because HIR-Bench's weighted_kendall_conflict
    ...
    # Crossover = the top of the low-conflict band in which DART essentially never wins.
    # Scan a sliding forward window; the crossover is the highest index i such that the
    # cells BELOW i have < 15% DART wins (the "mean-sufficient" band). Percentile = i/n.
    crossover_idx = 0
    for i in range(1, n):
...
def predict_regime(structure_reliability, preference_conflict, information_condition,
    ...
    run()).  DART is predicted only when conflict is in the upper part of the real
    distribution AND the query has reliable structure.
    """
    if information_condition in ("predicted_mean", "mean_only"):
        return "no_DART"  # information condition kills DART advantage (exp09 thesis)
...
def _observed_best_family(cand_pops, query_X, query_states, ctrl, minority, seed=0):
    """Which method family (DART vs mean) better covers the minority subpopulation.
    ...
    is the outcome, NOT energy regret (which DART optimizes and would make circular).
    """
...
def _acceptance_report(df, boundary):
    ...
    lines.append(f"HIR-Bench boundary transferred by percentile: crossover_percentile = "
                 ...
                 f"(fit on {boundary['n_cells']} grid cells; DART wins in "
    ...
    if degenerate:
        ...
        lines.append(
            ...
            "observed DART-minus-mean coverage margin across all tasks is below the 0.01 "
    ...
    lines.append(f"- predicted_mean → no-DART: {pm_ok:.0%}.")
    lines.append("- **This is a tautology, not evidence.** `predict_regime` returns 'no_DART' "
                 ...
                 "that mean-only predicted libraries offer no DART advantage rests on exp09, not "
                 "on this line.")
    ...
    lines.append("| dataset | observed DART-favoured | observed mean-sufficient | conflict (mean) | structure (mean) |")
    lines.append("|---|---|---|---|---|")
    ...
    lines.append("\n*Note:* the observed regime is defined by minority-state coverage (not energy "
                 "regret), so it does not mechanically favour DART. Where the real data places a "
...
def run(quick=False):
    ...
    log(f"  HIR boundary: crossover_percentile={boundary['conflict_crossover_percentile']:.3f}, "
        f"DART wins {boundary['frac_cells_dart_wins']:.1%} of {boundary['n_cells']} cells")
