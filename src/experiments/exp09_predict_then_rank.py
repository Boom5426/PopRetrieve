...
does swapping a mean/R² retrieval for PopRetrieve's distributional retrieval move the ground-truth
...
Outputs (results/exp09_predict_then_rank/):
    ...
    flip_vs_mean.csv            PopRetrieve vs mean/R² flip, overlap, Δrank (predictor held fixed)
...
def run(n_seeds=8, n_drugs=10, alphas=(0.5, 0.7, 0.9), lines=("K562", "A549", "MCF7"),
    ...
    # --- PopRetrieve vs mean/R2 flip (predictor held fixed) ---
    ...
    for pname in piv.index:
        ...
        log(f"  {pname:16s} best signature={best_sig:.3f}  best PopRetrieve={best_dart:.3f}  "
