...
EvalShift's central claim is a *divergence-gated* boundary: when a query population is internally
homogeneous, mean-signature retrieval is already optimal and EvalShift's distributional scorers
...
('covers-both', the only candidate that reproduces BOTH modes) against distractors under:
    ...
    global_energy   EvalShift energy-distance retrieval (distributional)
    coverage_worst  EvalShift worst-subpop coverage (the divergence-gated scorer)

The phase-diagram source is the per-cell Hit@1 (and MRR) for each scorer, plus the EvalShift
...
Outputs (results/exp11_synthetic_phase_diagram/):
    ...
    boundary_contour.csv        per alpha, the lambda* where EvalShift advantage crosses +0.05
...
def run(n_seeds=12, grid=9, lines=("K562",), n_total=300, n_distractors=20):
    ...
    # --- EvalShift advantage grid (energy - mean, coverage - mean) per (lambda, alpha) ---
    ...
    section("EXP11 HEADLINE — EvalShift energy advantage over mean_cosine (Hit@1 Δ)")
