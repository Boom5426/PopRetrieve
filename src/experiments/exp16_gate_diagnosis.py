...
ABSOLUTE RULES (anti-self-deception, per the execution spec):
  - Primary judge metrics are the two NON-CIRCULAR ones only. Never tune an
    energy-based / PopRetrieve-aligned metric to make PopRetrieve win.
...
# The PopRetrieve method vs the mean incumbent for per-query non-circular gaps.
...
def _load_exp12_perquery():
    ...
    and PopRetrieve-vs-mean gaps on the two non-circular metrics.
    ...
    # PopRetrieve and mean non-circular outcomes -> per-query gap
