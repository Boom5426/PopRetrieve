...
def main():
    ...
    if hi_sig_material:
        outcome = ("**A — conditional advantage holds.** In the highest-divergence stratum, "
                   "PopRetrieve significantly AND materially exceeds mean retrieval on a non-circular "
                   "metric.")
    elif hi_sig_trivial:
        outcome = ("**B(−) — statistically real but negligible.** In the highest-divergence "
                   "stratum PopRetrieve significantly exceeds mean retrieval on a non-circular metric, "
        ...
        outcome = ("**B — trend, sample-limited.** The highest-divergence stratum shows a "
                   "positive but non-significant PopRetrieve gap on a non-circular metric, and the "
        ...
        outcome = ("**C — chain broken in expression data.** The highest-divergence stratum "
                   "shows no positive non-circular PopRetrieve advantage. Changing the gate or the "
    ...
    if not aligned:
        ...
        lesion += ("even stratifying directly by true divergence — bypassing the gate entirely — "
                   "PopRetrieve shows no MATERIAL non-circular advantage in the highest-divergence "
    ...
    if outcome.startswith("**A"):
        impact = ("Claim 3/Claim 4 reconciliation: the gate (or the true-divergence stratifier) "
                  "DOES isolate a subset with a genuine non-circular PopRetrieve advantage. Report this "
        ...
        impact = ("Sharpens Claim 4 without overturning it. The mean→distribution advantage is "
                  "(a) real and divergence-monotone on the coverage proxy — PopRetrieve's sensitivity to "
