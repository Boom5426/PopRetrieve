...
subpopulation response divergence, not the gate), does DART beat mean retrieval on the
two NON-CIRCULAR metrics?

Q2 (stratified advantage):
  - Stratify all real per-query rows into quartiles of true_divergence.
  - In each stratum, paired DART-vs-mean gap on minority_state_coverage and moa_ndcg,
    with paired Wilcoxon signed-rank + Benjamini-Hochberg across strata.
  - Core question: in the HIGHEST-divergence stratum, is DART significantly > mean on a
    non-circular metric?
...
def _per_query_gaps():
    """One row per query: true_divergence + paired DART-mean gap on each non-circular metric.
...
def _paired_test(gaps):
    """Paired Wilcoxon on the DART-mean differences (gaps). Returns stat dict."""
...
def stratified(df):
    section("EXP17 Q2 — true-divergence-stratified DART-vs-mean on non-circular metrics")
...
def run(quick=False):
    ...
    df = _per_query_gaps()
    log(f"  loaded {len(df)} queries with paired DART/mean non-circular outcomes")
