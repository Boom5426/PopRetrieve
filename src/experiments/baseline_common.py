...
def query_divergence(nq: NormalizedQuery) -> float:
    ...
    the mean signature is unrepresentative (DART's regime).
    """
...
def dart_scores(q: dict, style: str, max_cells: Optional[int] = None,
    ...
    """The four DART scorers for a raw task dict (controlled or labeled style)."""
    if style == "controlled":
...
def per_query_metrics(vals: np.ndarray, gt_index: int,
    ...
    ``ref_vals`` : score vector of the reference ranker (DART) for flip / overlap / Δrank.
    """
