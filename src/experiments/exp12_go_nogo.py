...
def _paired_dart_vs_mean(perf, rmode):
    """Paired regret: mean_cosine vs best PopRetrieve method on the given recommendation subset.
    ...
    for dm in DART_METHODS:
        ...
        if len(common) < 5:
            continue
        red = ref.loc[common].values - dd.loc[common].values  # + => PopRetrieve lower regret
...
def _ndcg_mrr_advantage(perf, rmode):
    """Best PopRetrieve nDCG/MRR minus mean_cosine on the subset (leave_drug_out only)."""
...
def main():
    ...
    # C1: PopRetrieve lowers regret on DART_recommended subset (significant)
    ...
    # C2: PopRetrieve nDCG/MRR beats mean on DART_recommended (high-conflict reliable-structure)
    ...
    # C3: low-conflict correctly judged mean_sufficient (PopRetrieve does not win there)
    r_ms = _paired_dart_vs_mean(perf, "mean_sufficient")
    if r_ms:
        # PASS if PopRetrieve does NOT significantly beat mean here (regret reduction ~0 or n.s.)
        c3 = not (r_ms["median_regret_reduction"] > 0.05 and r_ms["wilcoxon_p"] < 0.05)
        crit["Go-3"] = (c3, f"mean_sufficient: PopRetrieve regret reduction={r_ms['median_regret_reduction']:+.4f}, "
                            f"p={r_ms['wilcoxon_p']:.3g} (should be ~0 / n.s.)")
    else:
        crit["Go-3"] = (True, "mean_sufficient subset small/empty — no spurious PopRetrieve win (vacuously ok)")
    ...
    # C5: >=1 real case where PopRetrieve changes top-k and covers a minority state mean missed
    ...
    crit["Go-5"] = (mino_gain >= 1, f"{mino_gain} queries where PopRetrieve covers a minority state "
    ...
    # A GO is only meaningful if PopRetrieve's advantage is CONCENTRATED in DART_recommended.
    ...
    L.append(f"- Non-circular nDCG (MoA-recovery) gain on DART_recommended = {ndcg_gain_rec:+.4f} "
             f"({'PopRetrieve helps' if ndcg_supports else 'PopRetrieve does NOT help under the non-circular outcome'})\n")
    L.append("### Reasoning\n")
    L.append("The phase-gate asks whether PopRetrieve's advantage appears in the realistic "
    ...
    if verdict.startswith("GO"):
        L.append("PopRetrieve clears the bar cleanly: it reduces welfare-decision regret specifically on "
        ...
        L.append("**The GO is metric-dependent and must be reported as such.** PopRetrieve reduces the "
                 ...
                 "1. *Metric alignment.* The welfare-regret proxy is energy-based, and PopRetrieve "
                 "optimizes distributional (energy/MMD) distance — so the regret metric is "
                 "partially aligned with PopRetrieve's objective. PopRetrieve also reduces this regret on the "
                 ...
                 "coverage (outcomes PopRetrieve does not directly optimize), PopRetrieve shows no consistent "
                 "gain over mean retrieval on this SciPlex3 leave-drug-out task (exp13: 0/37 tasks "
                 "PopRetrieve-favoured under minority coverage).\n\n"
                 "**Recommended positioning:** PopRetrieve's demonstrated contribution is *welfare-regret "
        ...
        L.append("PopRetrieve does not clear the bar under the non-circular outcomes. Reposition as a "
