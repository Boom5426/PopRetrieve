...
that is independent of the energy distance used by DART for selection.
...
If DART shows regret reduction only under (1) but not (2), then +0.119 is
purely circular: DART selects the drug with best energy, welfare IS energy,
so DART wins by definition.
"""
...
def main():
    ...
    # Regret reduction = mean_cosine_regret - DART_regret (positive = DART better)
    ...
    print(f"  Energy welfare:       {energy_reduction:.6f}  (frac DART better: {frac_better_energy:.3f})")
    print(f"  Delta-cosine welfare: {cosine_reduction:.6f}  (frac DART better: {frac_better_cosine:.3f})")
    ...
    if confirmed:
        ...
        print("  DART shows NO advantage under an oracle-independent welfare definition.")
    else:
        print("  DART retains some advantage even under alternative welfare.")
