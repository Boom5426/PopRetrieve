...
print(f"  DART mean: {dart_rows.mean_based_coverage.mean():.4f}")
print(f"  Mean mean: {mean_rows.mean_based_coverage.mean():.4f}")
print(f"  Gap (DART - mean): {mb_gap.mean():.4f} (median {np.median(mb_gap):.4f})")
print(f"  DART wins: {(mb_gap > 0).mean():.1%}")
...
print(f"  DART mean: {dart_rows.cell_level_coverage.mean():.4f}")
print(f"  Mean mean: {mean_rows.cell_level_coverage.mean():.4f}")
print(f"  Gap (DART - mean): {cl_gap.mean():.4f} (median {np.median(cl_gap):.4f})")
print(f"  DART wins: {(cl_gap > 0).mean():.1%}")

# MoA match
print(f"\nMoA match rate:")
print(f"  DART: {dart_rows.moa_match.mean():.3f}")
