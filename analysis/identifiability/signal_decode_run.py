...
"""signal_decode_run.py: DART-vs-mean pick divergence + drug attributes on SciPlex3."""
    ...
    for seed in SEEDS:
        ...
        for qi in range(N):
            ...
            rows.append(dict(
                ...
                regret_advantage=dw - mw,  # positive = DART lower regret
            ))
...
for f_ in ("tanimoto", "ntargets", "target_jaccard", "moa_match"):
    ...
    print(f"  {f_}: DART={d_:.4f} Mean={m_:.4f}", flush=True)
