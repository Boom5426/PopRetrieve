...
- `class_c_experiment.py`, ranks candidates by JUDGE energy vs mean cosine, scored
...
- `class_c_experiment.py`, `class_c_magnitude_control.py`: **SUPERSEDED, do not quote.**
  ...
  similarity), so JUDGE's top pick was the population *farthest* from the query while the
