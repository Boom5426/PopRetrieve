...
here and the v1 numbers must not be used.
  ...
  1. THE SIGN. `retrieval.metrics.score_energy` returns MINUS the energy distance, i.e. a
     ...
     `argmin(dart_scores)`) under the comment "lowest energy = top pick", so JUDGE's rank-1
     candidate was the population FARTHEST from the query. The mean-cosine baseline in the
     same scripts was ranked correctly (`argmax`). JUDGE was therefore ranked backwards and
     ...
     drugs, which is where v1's apparent +0.52 JUDGE-vs-potency correlation came from. It
